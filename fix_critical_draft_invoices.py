#!/usr/bin/env python3
"""
Fix CRITICAL Issue: Draft Invoices Showing as "Invoiced"
---------------------------------------------------------
Updates 8 orders that have draft (unvalidated) invoices but are marked as "invoiced"
These should be "to invoice" status instead.
...
def main():
    print("=" * 100)
    print("FIX CRITICAL ISSUE: Draft Invoices Marked as 'Invoiced'")
    print("=" * 100)
    ...
    # List of 8 orders with draft invoices marked as "invoiced"
    order_names = [
        'PS/07/4352',
        'PS/10/4525',
        'PS/10/4518',
    ...
    print(f"\nSearching for {len(order_names)} orders with draft invoice issues...")
    ...
    audit_log = {
        ...
        'database': db,
        'user': username,
        'operation': 'Fix Draft Invoices Marked as Invoiced',
        'orders_processed': []
    }
    
    print("\n" + "=" * 100)
    print("ORDERS TO FIX - DRAFT INVOICES SHOWING AS 'INVOICED'")
    print("=" * 100)
    ...
    for order in orders:
        ...
        if order['invoice_ids']:
            ...
            if draft_invoices and not posted_invoices and order['invoice_status'] == 'invoiced':
                ...
                print(f"   Current Status: {order['invoice_status']} (WRONG - has only DRAFT invoices)")
                ...
                audit_log['orders_processed'].append({
                    ...
                    'reason': 'Has only draft (unvalidated) invoices',
                    'draft_invoices': [{'name': inv['name'], 'state': inv['state']} for inv in draft_invoices]
                })
    ...
    print("\nThese orders will be updated from 'invoiced' to 'to invoice'")
    ...
    for order in orders_to_fix:
        try:
            result = models.execute_kw(
                db, uid, password,
                'sale.order', 'write',
                [[order['id']], {'invoice_status': 'to invoice'}]
            )
            
            if result:
                updated_count += 1
                print(f"✓ Updated {order['name']}: invoiced → to invoice")
    ...
    # Save audit log
    log_filename = f"fix_draft_invoices_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    for order in updated_orders:
        status_icon = "✓" if order['invoice_status'] == 'to invoice' else "✗"
