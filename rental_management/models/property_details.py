...
class PropertyDetails(models.Model):
    ...
    city = fields.Char(string='City (Text)', translate=True)
    ...
    crm_lead_ids = fields.One2many(
        ...
        'property_id',
        string="CRM Leads",
        help="All leads and inquiries related to this property"
    )
    ...
    def get_top_broker(self):
        ...
        for group in self.env['property.vendor'].read_group(
                [('is_any_broker', '=', True), ('company_id',
                                                'in', company_ids), ('stage', '=', 'sold')],
                ['broker_id'],
                ['broker_id'], limit=5):
            if group['broker_id']:
                name = self.env['res.partner'].sudo().browse(
