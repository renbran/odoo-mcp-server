...
export class DeepDatabaseCleanup {
     ...
     */
    async executeDeepCleanup(options) {
        ...
        try {
            ...
            const partnerResult = await this.removePartners(client, options.keepCompanyDefaults, options.dryRun);
            ...
            const invoiceResult = await this.removeInvoicesAndAccounting(client, options.dryRun);
            ...
            const purchaseResult = await this.removePurchaseOrders(client, options.dryRun);
            ...
            const calendarResult = await this.removeCalendarData(client, options.dryRun);
            ...
            // 9. Remove HR data
            const hrResult = await this.removeHRData(client, options.keepUserAccounts, options.dryRun);
            ...
            const logsResult = await this.removeLogsAndAttachments(client, options.dryRun);
            ...
            report.summary.totalRecordsRemoved = Object.values(report.summary)
                ...
                .reduce((a, b) => a + b, 0);
            if (!options.dryRun) {
                report.warnings.push('⚠️ IMPORTANT: All non-essential data has been removed. Backup was recommended before this operation.');
            }
            return report;
        }
        catch (error) {
     ...
     */
    async removePartners(client, keepDefaults = true, dryRun = false) {
        const details = [];
        let totalCount = 0;
        try {
            ...
            const allPartners = await client.search({
                model: 'res.partner',
                domain: (keepDefaults ? [['name', '!=', 'Your Company']] : []),
            ...
            if (allPartners.success && allPartners.data && allPartners.data.length > 0) {
                ...
                if (keepDefaults && partnerDetails.success && partnerDetails.data) {
                    toDelete = partnerDetails.data
                        .filter((p) => !systemPartnerNames.some((name) => p.name?.includes(name)))
                        .map((p) => p.id);
                }
     ...
     */
    async removeSalesDocuments(client, dryRun = false) {
        const details = [];
        let totalOrders = 0;
        try {
            // Remove sales orders
     ...
     */
    async removeInvoicesAndAccounting(client, dryRun = false) {
        const details = [];
        let invoiceCount = 0;
        let journalCount = 0;
        let accountCount = 0;
        try {
            ...
            const journals = await client.search({
                ...
                domain: [['type', 'not in', ['general', 'situation']]],
            });
            ...
            const accounts = await client.search({
                ...
                domain: [['code', 'not ilike', '1%']], // Keep accounts starting with 1
            });
     ...
     */
    async removePurchaseOrders(client, dryRun = false) {
        const details = [];
        let totalCount = 0;
        try {
     ...
     */
    async removeStockData(client, dryRun = false) {
        const details = [];
        let moves = 0;
        let products = 0;
        let locations = 0;
        ...
        try {
            ...
            const productProducts = await client.search({
                ...
                domain: [['create_date', '!=', false]], // Remove created products
            });
            if (productProducts.success &&
     ...
     * Remove CRM data
     */
    async removeCRMData(client, dryRun = false) {
        const details = [];
        let leads = 0;
        let opportunities = 0;
        try {
     ...
     */
    async removeProjectData(client, dryRun = false) {
        const details = [];
        let projects = 0;
        let tasks = 0;
        try {
     ...
     */
    async removeCalendarData(client, dryRun = false) {
        const details = [];
        let events = 0;
        let attendees = 0;
        try {
     ...
     * Remove HR data
     */
    async removeHRData(client, keepUserAccounts = true, dryRun = false) {
        const details = [];
        let employees = 0;
        let departments = 0;
        try {
            ...
            const employeeSearch = await client.search({
                model: 'hr.employee',
                domain: (keepUserAccounts ? [['user_id', '=', false]] : []),
            });
     ...
     */
    async removeLogsAndAttachments(client, dryRun = false) {
        const details = [];
        let totalCount = 0;
        try {
     ...
     */
    async identifyDefaultData(client) {
        const defaults = [];
        try {
...
//# sourceMappingURL=deep-cleanup.js.map