...
class UAELeaveAllocation(models.Model):
    ...
    @api.depends('leave_type_id', 'number_of_days', 'employee_id')
    def _compute_payment_amount(self):
        for record in self:
            # Get active contract for employee
            contract = False
            if record.employee_id and 'hr.contract' in self.env:
            ...
            if record.leave_type_id.payment_type == 'full':
                record.payment_amount = record.number_of_days * daily_wage
