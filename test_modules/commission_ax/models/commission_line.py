...
class CommissionLine(models.Model):
    ...
    # Enhanced Payment Tracking from PO/Bill
    po_payment_state = fields.Selection(
        string="PO Payment State",
        selection=[
            ('not_paid', 'Not Paid'),
        ...
        compute='_compute_po_payment_state',
        store=True,
        help="Payment state from linked purchase order"
    )
    ...
    bill_payment_state = fields.Selection(
        ...
        compute='_compute_bill_payment_state',
        store=True,
        help="Payment state from linked vendor bill"
    )

    actual_paid_amount = fields.Monetary(
        string="Actual Paid (from Bills)",
        compute='_compute_actual_paid_amount',
        store=True,
        currency_field='currency_id',
        help="Actual amount paid based on vendor bill payments"
    )
    ...
    is_fully_paid = fields.Boolean(
        ...
        store=True,
        help="True if commission is fully paid via PO/bill"
    )

    payment_completion_percentage = fields.Float(
        string="Payment Completion %",
        compute='_compute_payment_completion',
        store=True,
        help="Percentage of commission amount that has been paid"
    )
    ...
    @api.depends('purchase_order_id', 'purchase_order_id.invoice_status', 'purchase_order_id.order_line.invoice_lines.move_id.payment_state')
    def _compute_po_payment_state(self):
        """Compute payment state from purchase order"""
        for record in self:
            if not record.purchase_order_id:
                record.po_payment_state = False
                continue

            po = record.purchase_order_id
            # Get all bills from PO lines
            bills = po.order_line.mapped('invoice_lines.move_id').filtered(
                lambda m: m.move_type == 'in_invoice' and m.state != 'cancel'
            )

            if not bills:
                record.po_payment_state = 'not_paid'
            elif all(bill.payment_state == 'paid' for bill in bills):
                record.po_payment_state = 'paid'
            elif any(bill.payment_state == 'paid' for bill in bills):
                record.po_payment_state = 'partial'
            elif any(bill.payment_state == 'in_payment' for bill in bills):
                record.po_payment_state = 'in_payment'
            else:
                record.po_payment_state = 'not_paid'

    @api.depends('vendor_bill_id', 'vendor_bill_id.payment_state')
    def _compute_bill_payment_state(self):
        """Compute payment state from vendor bill"""
        for record in self:
            if record.vendor_bill_id and record.vendor_bill_id.payment_state:
                record.bill_payment_state = record.vendor_bill_id.payment_state
            else:
                record.bill_payment_state = False

    @api.depends('purchase_order_id', 'vendor_bill_id', 'po_payment_state', 'bill_payment_state')
    def _compute_actual_paid_amount(self):
        """Compute actual paid amount from bills"""
        for record in self:
            paid_amount = 0.0

            # Get paid amount from vendor bill
            if record.vendor_bill_id and record.vendor_bill_id.payment_state == 'paid':
                paid_amount = record.vendor_bill_id.amount_total

            # Get paid amount from PO bills
            elif record.purchase_order_id:
                bills = record.purchase_order_id.order_line.mapped('invoice_lines.move_id').filtered(
                    lambda m: m.move_type == 'in_invoice' and m.payment_state == 'paid'
                )
                paid_amount = sum(bills.mapped('amount_total'))

            record.actual_paid_amount = paid_amount

    @api.depends('actual_paid_amount', 'commission_amount')
    def _compute_payment_completion(self):
        """Compute payment completion status"""
        for record in self:
            if record.commission_amount:
                percentage = (record.actual_paid_amount / record.commission_amount) * 100
                record.payment_completion_percentage = min(percentage, 100.0)
                record.is_fully_paid = percentage >= 100.0
            else:
                record.payment_completion_percentage = 0.0
    ...
    def action_cancel(self):
        ...
        return {
            ...
            'params': {
                'title': _('Success'),
                'message': _('Commission(s) cancelled successfully!'),
                'type': 'success',
                'sticky': False,
    ...
    def action_reset_to_draft(self):
        ...
        return {
            ...
            'params': {
                'title': _('Success'),
                'message': _('Commission(s) reset to draft successfully!'),
                'type': 'success',
                'sticky': False,
                'next': {'type': 'ir.actions.act_window_close'},
            }
    ...
    def _create_purchase_order(self):
        ...
        # Map project/unit from originating sale order if available
        try:
            if getattr(self.sale_order_id, 'project_id', False):
                po_vals['project_id'] = self.sale_order_id.project_id.id
            if getattr(self.sale_order_id, 'unit_id', False):
                po_vals['unit_id'] = self.sale_order_id.unit_id.id
        except Exception:
            # Non-fatal: proceed without project/unit if not available
            pass
