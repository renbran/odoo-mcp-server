...
class SaleOrder(models.Model):
    ...
    def create(self, vals_list):
        """Override create to auto-populate commission rates from HR employee data"""
        for vals in vals_list:
            _logger.info("=== COMMISSION CREATE OVERRIDE CALLED ===")
            _logger.info(f"Input vals: {vals}")
            
            # Auto-populate commission rates for agents if source and partner are provided
            if 'source_id' in vals:
                _logger.info(f"Source ID found: {vals['source_id']}")
                for agent_field in ['agent1_partner_id', 'agent2_partner_id', 'manager_partner_id', 'director_partner_id']:
                    if agent_field in vals and vals.get(agent_field):
                        _logger.info(f"Processing {agent_field}: {vals[agent_field]}")
                        rate_field = agent_field.replace('_partner_id', '_rate')
                        # Only auto-populate if rate not already set
                        if rate_field not in vals or not vals.get(rate_field):
                            partner_id = vals[agent_field]
                            rate = self._get_commission_rate_for_partner(partner_id, vals.get('source_id'), agent_field)
                            _logger.info(f"Got rate for {agent_field}: {rate}")
                            if rate:
                                vals[rate_field] = rate
                                _logger.info(f"Set {rate_field} = {rate}")
            
            _logger.info(f"Final vals: {vals}")
        
        return super(SaleOrder, self).create(vals_list)
    
    def _get_commission_rate_for_partner(self, partner_id, source_id, agent_field):
        """Get commission rate for a partner based on employee profile and lead source"""
        if not partner_id:
            return None
        ...
        if not employee or not employee.is_agent:
            return None
        
        # Determine if it's a personal or business lead
        ...
        # Get rate based on agent field type and lead type
        if 'agent1' in agent_field or 'agent2' in agent_field:
            if employee.agent_type == 'primary':
                if is_personal_lead:
                    return employee.primary_agent_personal_commission
                else:
                    return employee.primary_agent_business_commission
            elif employee.agent_type == 'secondary':
                if is_personal_lead:
                    return employee.secondary_agent_personal_commission
                else:
                    return employee.secondary_agent_business_commission
        elif 'manager' in agent_field:
            if employee.agent_type == 'exclusive_rm':
                return employee.exclusive_rm_commission
            elif employee.agent_type == 'exclusive_sm':
                return employee.exclusive_sm_commission
            else:
                return employee.exclusive_rm_commission or 5.0
        elif 'director' in agent_field:
            if employee.agent_type == 'exclusive_sm':
                return employee.exclusive_sm_commission
            elif employee.agent_type == 'exclusive_rm':
                return employee.exclusive_rm_commission
            else:
                return employee.exclusive_sm_commission or 2.0
    ...
    def _onchange_agent1_partner(self):
        ...
        if self.agent1_partner_id:
            # Check if partner is linked to an employee via user
            employee = self.env['hr.employee'].search([
            ...
            if employee and employee.is_agent:
                ...
                if self.source_id:
                    ...
                    is_personal_lead = 'personal' in self.origin.lower() or 'referral' in self.origin.lower()
                
                # Set commission rate based on agent type and lead type
                if employee.agent_type == 'primary':
                    if is_personal_lead:
                    ...
                    if is_personal_lead:
                        self.agent1_rate = employee.secondary_agent_personal_commission
                    else:
                        self.agent1_rate = employee.secondary_agent_business_commission
                elif employee.agent_type == 'exclusive_rm':
                    self.agent1_rate = employee.exclusive_rm_commission
                elif employee.agent_type == 'exclusive_sm':
                    self.agent1_rate = employee.exclusive_sm_commission
                else:
                    # Default to primary agent commission if no specific type
                    self.agent1_rate = employee.primary_agent_commission

    @api.onchange('agent1_rate', 'agent1_calculation_base', 'agent1_commission_type', 'amount_untaxed', 'amount_total', 'source_id')
    def _onchange_agent1_commission(self):
        """Auto-calculate agent1 commission amount"""
        # Re-trigger partner onchange if source changes
        if self.source_id and self.agent1_partner_id:
            self._onchange_agent1_partner()
    ...
    @api.onchange('agent2_partner_id')
    def _onchange_agent2_partner(self):
        """Auto-populate agent2 commission rate from HR employee profile"""
        if self.agent2_partner_id:
            # Check if partner is linked to an employee via user
            employee = self.env['hr.employee'].search([
                ('user_id.partner_id', '=', self.agent2_partner_id.id)
            ], limit=1)
            ...
            if employee and employee.is_agent:
                ...
                if employee.agent_type == 'primary':
                    if is_personal_lead:
                        self.agent2_rate = employee.primary_agent_personal_commission
                    else:
                        self.agent2_rate = employee.primary_agent_business_commission
                elif employee.agent_type == 'secondary':
                    if is_personal_lead:
                        self.agent2_rate = employee.secondary_agent_personal_commission
                    else:
                        self.agent2_rate = employee.secondary_agent_business_commission
                elif employee.agent_type == 'exclusive_rm':
                    self.agent2_rate = employee.exclusive_rm_commission
                elif employee.agent_type == 'exclusive_sm':
                    self.agent2_rate = employee.exclusive_sm_commission
                else:
                    # Default to secondary agent commission if no specific type
                    self.agent2_rate = employee.secondary_agent_commission

    @api.onchange('agent2_rate', 'agent2_calculation_base', 'agent2_commission_type', 'amount_untaxed', 'amount_total', 'source_id')
    def _onchange_agent2_commission(self):
        """Auto-calculate agent2 commission amount"""
        # Re-trigger partner onchange if source changes
        if self.source_id and self.agent2_partner_id:
            self._onchange_agent2_partner()
    ...
    def _onchange_manager_partner(self):
        """Auto-populate manager commission rate from HR employee profile"""
        if self.manager_partner_id:
            # Check if partner is linked to an employee via user
            employee = self.env['hr.employee'].search([
                ('user_id.partner_id', '=', self.manager_partner_id.id)
            ], limit=1)
            
            if employee and employee.is_agent:
                # Managers typically get exclusive_rm commission rate
                if employee.agent_type == 'exclusive_rm':
                    self.manager_rate = employee.exclusive_rm_commission
                elif employee.agent_type == 'exclusive_sm':
                    self.manager_rate = employee.exclusive_sm_commission
                else:
                    # Default manager commission
                    self.manager_rate = employee.exclusive_rm_commission or 5.0

    @api.onchange('manager_rate', 'manager_calculation_base', 'manager_commission_type', 'amount_untaxed', 'amount_total', 'source_id')
    def _onchange_manager_commission(self):
        """Auto-calculate manager commission amount"""
        # Re-trigger partner onchange if source changes
        if self.source_id and self.manager_partner_id:
            self._onchange_manager_partner()
    ...
    @api.onchange('director_partner_id')
    def _onchange_director_partner(self):
        """Auto-populate director commission rate from HR employee profile"""
        if self.director_partner_id:
            # Check if partner is linked to an employee via user
            employee = self.env['hr.employee'].search([
                ('user_id.partner_id', '=', self.director_partner_id.id)
            ], limit=1)
            
            if employee and employee.is_agent:
                # Directors typically get exclusive_sm commission rate
                if employee.agent_type == 'exclusive_sm':
                    self.director_rate = employee.exclusive_sm_commission
                elif employee.agent_type == 'exclusive_rm':
                    self.director_rate = employee.exclusive_rm_commission
                else:
                    # Default director commission
                    self.director_rate = employee.exclusive_sm_commission or 2.0

    @api.onchange('director_rate', 'director_calculation_base', 'director_commission_type', 'amount_untaxed', 'amount_total', 'source_id')
    def _onchange_director_commission(self):
        """Auto-calculate director commission amount"""
        # Re-trigger partner onchange if source changes
        if self.source_id and self.director_partner_id:
            self._onchange_director_partner()
