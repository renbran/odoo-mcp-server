...
export class DashboardChartWrapper extends Component {
  ...
  setup() {
    ...
    this.onDownloadImage = async (ev) => {
      var self = this;
      var default_todo = false;
      if (
        this.state.chart_type == "to_do" &&
        this.state.recordSets.layout_type == "default"
      ) {
        default_todo = true;
      }
      if (["kpi", "tile"].includes(this.state.chart_type) || default_todo) {
    ...
    this.update_chart = async (chartId, chartType, domain) => {
      ...
      if (chart_data.type && chart_data.type == "action") {
        chart_data.action["help"] = this.stripHtml(chart_data.action.help);
