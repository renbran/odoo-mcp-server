...
class Dashboard(models.Model):
                        ...
                        0,
                        {
                            ...
                            "action_line_ids": [
                                (
                                    ...
                                    {
                                        "name": action_line.get("name"),
                                        "active_record": action_line.get(
                                            "active_record"
                                        ),
                                    },
                                )
                                for action_line in (action.get("action_line_ids") or [])
                            ],
                        },
                    )
                    for action in (payload.get("todo_action_ids") or [])
                ],
            }
            multiplier_list = []
    ...
    def get_charts_details(self):
        ...
        try:
            ...
            if self.company_id and self.env.company.id != self.company_id.id:
                return [1, [], "", False]
        except Exception:
            return [1, [], "", False]
        chart_data_list = []
