...
class DashboardChart(models.Model):
    ...
    def get_kpi_data(self, conf_obj):
        ...
        if kcmp_type == "sum":
            ...
            prepared_data["count2"] = "%s %s" % (symbol, round(count2, 2))
    ...
    def get_todo_data(self, conf_obj):
        ...
        if conf_obj.limit_record == 0:
            activities = self.env["mail.activity"].search(
                activities_domain,
                order="%s %s"
                % (self.date_filter_field_id.name, conf_obj.sort_order or ""),
            )
        else:
            activities = self.env["mail.activity"].search(
                activities_domain,
    ...
    def _build_final_response(
        ...
        if self.item_action_id:
            ...
            try:
                ...
                raise ValidationError(
                    _("Invalid action selected. Please select another action.")
                )
    ...
    def get_measurement_group_data(self, conf_obj):
        ...
        def _sort_key(sr):
            val = getattr(sr, conf_obj.sort_field)
        ...
        if conf_obj.sort_order and conf_obj.sort_field:
            ...
            # )
            sorted_record = records.filtered(
                lambda rft: getattr(rft, conf_obj.sort_field) is not None
            ).sorted(
                key=_sort_key,
                reverse=(conf_obj.sort_order == "desc"),
            )
                ...
                for key, value in row.items():
                    if isinstance(value, list):
                        if len(value) != 0:
                            ...
                            # split_key_val = key.split('- ')
                            # split_key = key
                            # if split_key_val and len(split_key_val) > 1:
                            #     split_key = split_key_val[1]
                            if (
